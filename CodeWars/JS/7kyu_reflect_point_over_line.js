...
function reflect([x, y], [m, b]) {
    return [
        (-2 * m * b - m ** 2 * x + 2 * m * y + x) / (m ** 2 + 1),
        (2 * b + m ** 2 * y + 2 * m * x - y) / (m ** 2 + 1),
    ];
}
    ...
    describe('Solution test', () => {
        ...
        const sampleTests = [
            ...
            [
                [44, 123123],
                [44, 334],
                [5534.482188951988, 122998.21631388746],
            ],
            [
                [-2, -3],
                [-1, 0],
        ...
        function act(point, line, expected) {
            ...
            assert(
                actual.length == 2,
                'returned value should contain X and Y coordinates',
            );
            for (let i = 0; i < 2; i++) {
